...
describe("AgentsPage", () => {
  ...
  it("reuses preloaded core and sessions when loading issue signals", async () => {
    mockedDashboardData = {
      overview: {
        topBar: {
          appName: "OpenClaw 控制中心",
          ...
          timezone: "Asia/Shanghai",
          statusSummary: "存在 agent 相关线索。",
          channelSummary: { online: 1, total: 1 },
          agentSummary: { active: 1, total: 1 },
          alertsToday: 0,
          primaryModel: "openai/gpt-5.4"
        }
      },
      agents: [{ id: "writer", workspace: "/tmp/writer", role: "main" }],
      config: {
        agents: {
          defaults: {
    ...
    mockedIssueSignals = {
      ...
      logs: { excerpts: [], tokens: [], references: [], relatedSessionKeys: [], relatedAgentIds: [] }
    };
    mockedIssues = [];
    mockedSessionsSnapshot = { count: 0, sessions: [] };

    const { default: AgentsPage } = await import("@/app/agents/page");
    await AgentsPage();

    expect(loadIssueSignals).toHaveBeenCalledWith(
      expect.objectContaining({
        core: mockedDashboardData,
        sessions: mockedSessionsSnapshot
      })
    );
  });
});
